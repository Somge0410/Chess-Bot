...
std::pair<double,Move> Engine::negamax(Board& board, int depth, double alpha, double beta, int ply){
    ...
    if (depth==0)
    {
        double q_score=quiescence_search(board,alpha,beta,ply+1);
        return {q_score,Move()};
    }
...
uint64_t Engine::perft_driver(Board& board, int depth){
    if (depth==0){
...
void Engine::perft_test(Board& board, int depth) {
    ...
    uint64_t nodes_found = perft_driver(board, depth);
